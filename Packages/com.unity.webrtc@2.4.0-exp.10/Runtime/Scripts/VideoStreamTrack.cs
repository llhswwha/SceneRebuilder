...
namespace Unity.WebRTC
{
    ...
    public class VideoStreamTrack : MediaStreamTrack
    {
        ...
        public override void Dispose()
        {
            ...
            {
                Debug.LogErrorFormat("m_source:{0} m_destTexture:{1}"
                    , m_source==null,m_destTexture==null);
                try
                {
                    if (m_source != null)
                    {
                        ...
                        {
                            Debug.LogError("VideoStramTrack.RenderTexture.active Exception:" + exp.ToString());
                        }
                    ...
                    if(m_destTexture!=null) WebRTC.DestroyOnMainThread(m_destTexture, 0.1f);
                    m_renderer?.Dispose();
                    m_source?.Dispose();
                }catch(Exception e)
                {
                    Debug.LogError("VideoStramTrack Exception:"+e.ToString());
                }
