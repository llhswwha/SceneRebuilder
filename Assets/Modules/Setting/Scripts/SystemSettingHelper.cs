...
public class SystemSettingHelper : MonoBehaviour
{
    ...
    public static SceneLoadSetting sceneLoadSetting;

    /// <summary>
    /// 版本号
    /// </summary>
    public string versionNum = "1.0.20";
    public static SystemSettingHelper _instance;
    ...
    public static SystemSettingHelper instance
    {
        get
        {
            if (_instance == null)
            {
                _instance = GameObject.FindObjectOfType<SystemSettingHelper>();
            }
            if (_instance == null)
            {
                GameObject insGo = new GameObject(typeof(SystemSettingHelper).Name);
                _instance = insGo.AddComponent<SystemSettingHelper>();
            }
            return _instance;
        }
    }
    ...
    public static void GetSystemSetting(Action onComplete=null)
    {
        ...
        if (systemSetting == null)
        {
            ...
            Debug.LogError("GetSystemSetting Load:" + path);
            if (!File.Exists(path))
            {
    ...
    private static void InitSetting()
    {
        ...
        sceneLoadSetting = systemSetting.SceneLoadSetting;
        ///如果版本号不一致，自动更新
