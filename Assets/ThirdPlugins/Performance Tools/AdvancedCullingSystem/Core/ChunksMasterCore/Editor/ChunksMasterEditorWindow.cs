...
namespace AdvancedCullingSystem.ChunksMaster
{
    public class ChunksMasterEditorWindow : EditorWindow
    {
        ...
        private void OnSceneView(SceneView sceneView)
        {
            ...
            UnityEditorHelper.SetFunction(DrawFunction.OnSceneView);
            ...
            foreach (var go in _selectedObjects)
            {
                ...
                UnityEditorHelper.matrix = Matrix4x4.TRS(go.transform.position, go.transform.rotation, go.transform.lossyScale);

                foreach (var bounds in _chunksMaster.bounds[index])
                    UnityEditorHelper.DrawWireCube(bounds.center, bounds.size);
            }

            UnityEditorHelper.matrix = Matrix4x4.identity;
        }
