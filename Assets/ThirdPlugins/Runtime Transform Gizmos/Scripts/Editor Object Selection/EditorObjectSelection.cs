...
namespace RTEditor
{
    ...
    public class EditorObjectSelection : MonoSingletonBase<EditorObjectSelection>
    {
        ...
        public void Skode_Press(GameObject go)
        {
            _selectedObjects.Clear();

            _selectedObjects.Add(go);
            //_selectedObjects.Add(GameObject.Find("Button/Sphere"));
            //_selectedObjects.Add(GameObject.Find("Button/Cylinder"));
        ...
        private bool CanOperate()
        {
            ...
            //把!SceneGizmo.Instance.IsHovered()的判断条件去掉，为了让右上角的小坐标轴和底下的英文去掉
            return gameObject.activeSelf && enabled &&!EditorGizmoSystem.Instance.IsActiveGizmoReadyForObjectManipulation();
        }
        ...
        private bool CanPerformMultiSelect()
        {
            ...
            //把!SceneGizmo.Instance.IsHovered()的判断条件去掉，为了让右上角的小坐标轴和底下的英文去掉
            return ObjectSelectionSettings.CanMultiSelect && gameObject.activeSelf && enabled && InputDevice.Instance.IsPressed(0) 
                && _objectSelectionRectangle.IsVisible;
        }
        ...
        #region AddInSiHuiFactory 针对四会热电厂的修改
        /// <summary>
        /// 是否能取消选中
        /// </summary>
        /// <returns></returns>
        private bool IsCanDeselect()
        {
            // bool isEditMode = ObjectAddListPanel.IsEditMode;
            // if (!isEditMode) return true;
            // else
            {
                GameObject pickedGameObject = PickObjectInScene();
                if (pickedGameObject == null) return false;
                else return true;
            }
        }
        /// <summary>
        ...
        {
            return ObjectSelectionSettings.SelectableLayers;
        }
        #endregion
        /// <summary>
        ...
        private void OnInputDeviceFirstButtonDown()
        {
            ...
            if (CanOperate() && ObjectSelectionSettings.CanClickSelect)
            {
                ...
                if (!_multiDeselectShortcut.IsActive() && !_appendToSelectionShortcut.IsActive()&&IsCanDeselect()) ClearSelection(true, ObjectDeselectActionType.ClearClickAir);
