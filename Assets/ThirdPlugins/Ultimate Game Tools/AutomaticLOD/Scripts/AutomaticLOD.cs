...
public class AutomaticLOD : MonoBehaviour
{
    ...
    public static LevelsToGenerate DefaultLevelCount= LevelsToGenerate._4;

    [SerializeField]
    private LevelsToGenerate m_levelsToGenerate = DefaultLevelCount;
  ...
  void OnWillRenderObject()
  {
    ...
    void OnWillRenderObject()
    {
        ...
        if (Camera.current != null && Camera.current.enabled == false || m_originalMesh == null)
        {
            return;
        }
        ...
        if (automaticLOD != null)
        {
            if (IsRootOrBelongsToLODTree(automaticLOD, root))
            {
                if (automaticLOD.m_meshSimplifier != null)
                {
                    ...
                    //Debug.Log($"ComputeLODMeshRecursive fAmount:{fAmount},nVertexCount:{nVertexCount},OriginalMeshUniqueVertexCount:{automaticLOD.m_meshSimplifier.GetOriginalMeshUniqueVertexCount()}");
    ...
    {
        //Debug.LogError($"SetupLODGroupRecursive root:{root},gameObject:{gameObject},renderers:{renderers.Count}");
            ...
            if (IsRootOrBelongsToLODTree(automaticLOD, root))
            {
                if (automaticLOD.m_listLODLevels != null)
                {
                    ...
                    for (int i = 0; i < automaticLOD.m_listLODLevels.Count; ++i)
                    {
                        ...
                        //Debug.LogError($"data[{i}]2:{data}");
                    }


                    Renderer originalRenderer = gameObject.GetComponent<Renderer>();
                    //Debug.LogError($"originalRenderer:{originalRenderer}");
        ...
        //Debug.LogError($"gameObject.transform.childCount:{gameObject.transform.childCount}");
        for (int nChild = 0; nChild < gameObject.transform.childCount; nChild++)
        {
