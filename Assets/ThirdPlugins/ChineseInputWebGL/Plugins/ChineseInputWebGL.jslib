var ChineseInputWebGL = {
	...
	InputShow: function(GameObjectName_,inputID_,v_,fontsizeT_,indexStr_,inputRectStr_)
	{
		...
		var mode=inputRectArr[0];
		var posX = inputRectArr[1];
		var posY = inputRectArr[2];
		var width = inputRectArr[3];
		var height = inputRectArr[4];
		var casWidth=inputRectArr[5];
		var casHeight=inputRectArr[6];
		...
		if(offsetX>0)
		{		
			console.log("offsetX>0 posX:"+posX+" offsetX:"+offsetX);
			posX=posX-offsetX;
		}
		if(offsetY>0)
		{
			console.log("offsetY>0 posY:"+posY+" offsetY:"+offsetY);
			posY=posY-offsetY;
		}
		*/
		var scale=1;
		if(mode==1)
		{
			console.log("this.webglcontentSizeX"+this.webglcontentSizeX+" casWidth:"+casWidth+" scale:"+this.webglcontentSizeX/casWidth);
			scale=this.webglcontentSizeX/casWidth;
		}
		posX*=scale;
		posY*=scale;
		width*=scale;
		height*=scale;
		var xTemp=this.webglcontentSizeX/2+posX-width/2;
		var yTemp=this.webglcontentSizeY/2-posY-height/2;

		console.log("PosX:"+posX+" posY:"+posY+" width:"+width+" height:"+height+" xTemp:"+xTemp+" yTemp:"+yTemp);
		input.style.width=width+"px";
		input.style.height=height+"px";
		input.style.left =xTemp+"px";//左边距
		//posY=posY-60;//测试
		input.style.top =yTemp+"px";//上边距
		input.value = v;
