...
{   
    ...
    public class AssetDB<TID> : IAssetDB<TID> where TID : struct
    {
        ...
        {
            Debug.Log($"AssetDB.FromID id:{id}");
            UnityObject result;
        ...
        public virtual void RegisterSceneObject(TID id, UnityObject obj)
        {
            ...
            Debug.Log($"AssetDB.RegisterSceneObject id:{id} obj:{obj} instanceId:{instanceId}");
    ...
    public class AssetDB : IAssetDB
    {
        ...
        {
            //Debug.Log($"AssetDB._RegisterSceneObject id:{id} obj:{obj}");
            int intId = ToInt(id);
        ...
        public T FromID<T>(long id) where T : UnityObject
        {
            ...
            if(IsStaticResourceID(id))
            {
                //Debug.Log($"AssetDB.FromID1 id:{id} PersistentIDtoObj:{m_mapping.PersistentIDtoObj.Count}");
                UnityObject obj;
                int persistentID = ToInt(id);
            ...
            {
                //Debug.Log($"AssetDB.FromID2 id:{id} PersistentIDtoObj:{m_persistentIDToSceneObject.Count}");
                UnityObject obj;
                int persistentID = ToInt(id);
            ...
            {
                //Debug.Log($"AssetDB.FromID3 id:{id} PersistentIDtoObj:{m_persistentIDToDynamicResource.Count}");
                UnityObject obj;
                int persistentID = ToInt(id);
                if (m_persistentIDToDynamicResource.TryGetValue(persistentID, out obj))
                {
