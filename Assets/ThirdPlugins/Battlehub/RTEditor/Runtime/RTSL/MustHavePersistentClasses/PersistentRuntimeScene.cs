...
namespace Battlehub.RTSL.Battlehub.SL2
{
    ...
    public class PersistentRuntimeScene<TID> : PersistentRuntimePrefab<TID>, ICustomSerialization
    {
        ...
        {
            Debug.Log($"PersistentRuntimeScene.ReadFromImpl obj:{obj}");
        ...
        {
            //Debug.Log($"PersistentRuntimeScene.WriteToImpl 1 obj:{obj}");
            ClearReferencesCache();
            ...
            DestroyGameObjects(scene);
            //Debug.Log($"PersistentRuntimeScene.WriteToImpl 2 Descriptors:{Descriptors.Length}");
            ...
            if (AssetIds != null)
            {
                ...
                //Debug.Log($"PersistentRuntimeScene.WriteToImpl 3 AssetIds:{AssetIds.Length} factory:{factory}");
            ...
            //ShowTest(idToUnityObj, "PersistentRuntimeScene.WriteToImpl 4 RegisterSceneObjects1 ");
            m_assetDB.RegisterSceneObjects(idToUnityObj);
            //ShowTest(idToUnityObj, "PersistentRuntimeScene.WriteToImpl 5 RegisterSceneObjects2 ");
            ...
            if (assetInstances != null)
            {
                for (int i = 0; i < assetInstances.Length; ++i)
                {
                    ...
                    {
                        PersistentObject<TID> asset = Assets[i];
                        //Debug.Log($"PersistentRuntimeScene.WriteToImpl>WriteTo[{i+1}] asset:{asset}");
            ...
            //ShowTest(idToUnityObj, "PersistentRuntimeScene.WriteToImpl 6 WriteTo ");

            ShowTest(idToUnityObj, "PersistentRuntimeScene.WriteToImpl 7 RestoreDataAndResolveDependencies1 ");
            RestoreDataAndResolveDependencies();
            ShowTest(idToUnityObj, "PersistentRuntimeScene.WriteToImpl 8 RestoreDataAndResolveDependencies2 ");

            //ShowObjectParents(idToUnityObj);

            m_assetDB.UnregisterSceneObjects();

            ClearReferencesCache();

            //ShowTest(idToUnityObj, "PersistentRuntimeScene.WriteToImpl 4");


            GameObjects.Clear();
            int i1 = 0;
            foreach (var item in idToUnityObj)
            {
                UnityObject assetInstance = item.Value;
                if (assetInstance is GameObject)
                {
                    GameObject go = assetInstance as GameObject;
                    //Debug.Log($"PersistentRuntimeScene.WriteToImpl>WriteTo idToUnityObj[{i1 + 1}]1 assetInstance:{assetInstance} parent:{go.transform.parent} ");
                    GameObjects.Add(go);
                }
                else if (assetInstance is Transform)
                {
                    Transform t = assetInstance as Transform;
                    //Debug.Log($"PersistentRuntimeScene.WriteToImpl>WriteTo idToUnityObj[{i1 + 1}]2 assetInstance:{assetInstance} parent:{t.parent} 1");
                }
                else
                {
                    //Debug.Log($"PersistentRuntimeScene.WriteToImpl>WriteTo idToUnityObj[{i1 + 1}]3 assetInstance:{assetInstance} 1");
                }
                i1++;
            }
        ...
        private void ShowObjectParents(Dictionary<TID, UnityObject> idToUnityObj)
        {
            int i1 = 0;
            foreach (var item in idToUnityObj)
            {
        ...
        private void ShowTest(Dictionary<TID, UnityObject> idToUnityObj,string tag)
        {
            int id = 0;
            string log = "";
            foreach (var item in idToUnityObj)
            {
                id++;
                log += $"{tag} [{id}] key:{item.Key} value:{item.Value}\n";
                //Debug.Log(log);
            }
            Debug.Log(log);
        }
        ...
        {
            //Debug.Log("DestroyGameObjects");
            if (UnityObject.FindObjectOfType<RTSLAdditive>())
            {
                //Debug.Log("DestroyGameObjects1");
                return;
            }

            GameObject[] rootGameObjects = scene.GetRootGameObjects();
            //Debug.Log($"DestroyGameObjects2 rootGameObjects:{rootGameObjects.Length}");
            for (int i = 0; i < rootGameObjects.Length; ++i)
            {
                GameObject rootGO = rootGameObjects[i];
                //Debug.Log($"DestroyGameObjects3[{i+1}] rootGO:{rootGO}");
        ...
        {
            Debug.Log($"PersistentRuntimeScene.Serialize ");
        ...
        {
            Debug.Log($"PersistentRuntimeScene.Deserialize ");
