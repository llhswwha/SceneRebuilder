...
namespace Battlehub.RTSL.Battlehub.SL2
{
    ...
    public class PersistentRuntimePrefab<TID> : PersistentObject<TID>
    {
        ...
        {
            //Debug.Log($"PersistentRuntimePrefab.CreateGameObjectWithComponents Start parent:{parent} |PersistentID:{descriptor.PersistentID}|typeMap:{typeMap} |descriptor:{descriptor}");
            ...
            //go.name += "_Test1";
            //go.AddComponent<EnableDisableDebuger>();
            ...
            //Debug.Log($"PersistentRuntimePrefab.CreateGameObjectWithComponents End go:{go} objGo:{objGo}");
        ...
        protected void RestoreDataAndResolveDependencies()
        {
            ...
            for (int i = 0; i < Data.Length; ++i)
            {
                ...
                //Debug.Log($"PersistentRuntimePrefab.RestoreDataAndResolveDependencies[{i+1}] 11 id:{id} obj:{obj} data:{data} ");
                if (obj == null)
                {
                ...
                data.WriteTo(obj);
                //Debug.Log($"PersistentRuntimePrefab.RestoreDataAndResolveDependencies[{i + 1}] 12 id:{id} obj:{obj} data:{data} ");
            ...
            //Debug.Log($"PersistentRuntimePrefab.RestoreDataAndResolveDependencies Length:{Data.Length} goList:{goList.Count} goActivationList:{goActivationList.Count} deferredIndices:{deferredIndices.Count}");
            ...
            for(int i = 0; i < deferredIndices.Count; ++i)
            {
                ...
                UnityObject obj = FromID<UnityObject>(id);
                //Debug.Log($"PersistentRuntimePrefab.RestoreDataAndResolveDependencies2[{i + 1}] id:{id} obj:{obj} data:{data} ");
                data.WriteTo(obj);
                //Debug.Log($"PersistentRuntimePrefab.RestoreDataAndResolveDependencies3[{i + 1}] id:{id} obj:{obj} data:{data} ");
            }
        ...
        {
            //Debug.Log($"PersistentRuntimePrefab.AddComponent go:{go} componentType:{componentType} componentDescriptor:{componentDescriptor.Name}");
            Component component;
        ...
        {
            //Debug.Log($"PersistentRuntimePrefab.CreateDescriptorAndData go:{go}");
            if (go.GetComponent<RTSLIgnore>())
            {
                //Do not save persistent ignore objects
                return null;
