...
namespace Battlehub.RTSL
{
    ...
    public abstract class Project<TID> : MonoBehaviour
    {
        ...
        {
            Debug.Log("Project.CreateNewScene");
        ...
        {
            Debug.Log($"Project._OpenProject project:{project}");
        ...
        {
            Debug.Log($"Project.PersistentDescriptorsToPrefabPartItems descriptors:{descriptors.Length} prefabParts:{prefabParts.Count} includeRoot:{includeRoot}");
            if (descriptors == null)
            {
                return;
            }
        ...
        {
            //Debug.Log($"Project.BeginResolveDependencies");
        ...
        {
            Debug.Log($"Project.OnLoadCompleted rootAssetItems:{rootAssetItems.Length} assetItems:{assetItems.Length}");
                    ...
                    if (m_assetDB.IsDynamicResourceID(assetItemId))
                    {
                        ...
                        if (persistentObject != null)
                        {
                            if (persistentObject is PersistentRuntimeScene<TID>)
                            {
                                ...
                                var descriptor0 = persistentScene.Descriptors[0];
                                persistentScene.CreateGameObjectWithComponents(m_typeMap, descriptor0, idToObj, null);
                            }
            ...
            if (sceneObject != null)
            {
                var objs = sceneObject.WriteTo(SceneManager.GetActiveScene()) as List<GameObject>;
                ao.Result = objs.ToArray();
                Debug.Log($"Project.OnLoadCompleted1 objs:{objs.Count}");
            }

            Error error = new Error(Error.OK);
            UnityObject[] result = rootAssetItems.Select(rootItem => m_assetDB.FromID<UnityObject>(GetID(rootItem))).ToArray();
            Debug.Log($"Project.OnLoadCompleted2 result:{result.Length} sceneObject:{sceneObject}");
            if (callback != null)
            {
                callback(error, result);
            }
        ...
        private void _GetValue<T>(string key, ProjectEventHandler<T> callback, ProjectAsyncOperation<T> ao)
        {
            Type objType = typeof(T);
             if(isDebug){
                Debug.Log("_GetValue Start:"+objType);
            }
            Type persistentType = m_typeMap.ToPersistentType(objType);
            if (persistentType == null)
            {
            ...
            {
                if(isDebug){
                    Debug.Log("after m_storage.GetValue start:"+getValueError+"|"+persistentObject);
                }   
                if (getValueError.HasError)
                {
                    if(isDebug){
                        Debug.Log("getValueError.HasError");
                    }  
                ...
                if(isDebug){
                    Debug.Log("after m_storage.GetValue end:"+result);
                }  
            });
        }
    ...
    public class Project : Project<long>, IProject
    {
        ...
        {
            Debug.Log("Project.LoadLibraryWithSceneDependencies");
            LoadBuiltinLibrary(() =>
            {
        ...
        {
            //Debug.Log($"Project.LoadBuiltinLibrary m_assetDB:{m_assetDB}");
            if (!m_assetDB.IsLibraryLoaded(AssetLibraryInfo.BUILTIN_FIRST))
            {
                Debug.Log($"Project.LoadBuiltinLibrary1 m_assetDB:{m_assetDB} m_builtInLibrary:{m_builtInLibrary}");
                LoadBuiltinLibrary(m_builtInLibrary, AssetLibraryInfo.BUILTIN_FIRST, callback);
            }
            else
            {
                Debug.Log($"Project.LoadBuiltinLibrary2 m_assetDB:{m_assetDB} m_builtInLibrary:{m_builtInLibrary}");
                callback();
            }
        }
