...
namespace Battlehub.RTSL
{
    public class UnityObjectFactory : IUnityObjectFactory
    {
        ...
        {
            //Debug.Log($"UnityObjectFactory.CanCreateInstance type:{type} surrogate:{surrogate}");
        ...
        {
            Debug.Log($"UnityObjectFactory.CreateInstance1 type:{type}");
            Type persistentType = m_typeMap.ToPersistentType(type);
            IPersistentSurrogate surrogate = null;
            if (persistentType != null)
            {
        ...
        {
            //Debug.Log($"UnityObjectFactory.CreateInstance2 type:{type} surrogate:{surrogate}");
            if(type == null)
            {
