...
namespace Battlehub.RTHandles
{
    ...
    {
        public static bool GetIsStatic(GameObject obj)
        {
            bool isS = obj.isStatic;
            //Debug.Log("GetIsStatic:"+ isS +" Object:"+obj);
            //isS = false;
            return isS;
        }

        public bool IsAllNoStatic = false;//实际项目中全部能够修改，就算设置成static，也是动态创建的，能够移动的，实际上static无效，有时间修改模型预设，没时间统一都是非static的
        ...
        protected virtual Transform[] Targets_Internal
        {
            ...
            set
            {
                ...
                bool haveStatic = m_activeTargets.Any(target => GetIsStatic(target.gameObject));
                // haveStatic = IsAllNoStatic;
                // Debug.Log("haveStatic:"+haveStatic);
                if (haveStatic)
                {
