...
public class MeshInfo
{
    ...
    public void Update()
    {
        ...
        if (!m_Mesh)
        {
            Debug.LogError("!m_Mesh");
                return;
        }

        // if (!(m_MeshReadable = m_Mesh.isReadable))
        // {
        //     Debug.LogError("!(m_MeshReadable = m_Mesh.isReadable)");
        //         return;
        // }
        ...
        // for (int i = 0; i < m_Mesh.vertexCount && i<10; i++)
        // {
        //     m_vertexFeature += m_Mesh.vertices[i];
        // }

        //Debug.Log(m_Mesh.name+"|"+ m_Features);
    }

    public string m_meshFeature="";

    public string m_vertexFeature = "";
...
public static class InternalMeshUtil
{
    ...
    public static bool canCreateSetIndicesDelegate = true;

    static void CreateSetIndicesDelegate()
    {
        try
        {
            if (canCreateSetIndicesDelegate == false) return;
            // See ILSpy for this hidden feature
        ...
        {
            Debug.LogWarning("CreateSetIndicesDelegate:"+ex);
            canCreateSetIndicesDelegate = false;//һ�ξͺ�
        }
    }
    ...
    public static void SetIndices(Mesh m, List<int> buffer, MeshTopology topology, int submesh, bool recalculate)
    {
        try
        {
            ...
            if (_SetIndices == null)
            {
                //Debug.LogWarning("SetIndices: _SetIndices == null");
                m.SetIndices(buffer, topology, submesh, recalculate);
            }
            else
            {
        ...
        {
            Debug.LogWarning("SetIndices:" + ex);
        }
    }
