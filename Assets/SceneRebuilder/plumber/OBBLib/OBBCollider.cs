...
public class OBBCollider : MonoBehaviour
{
    ...
    public static float GetObbRTDistance(GameObject result, GameObject original)
    {
        ...
        var vs2 = obb2.CornerPointsVector3();
        //MeshHelper.GetVertexDistanceEx(vs1, vs2,)
        var dis = AcRigidTransformHelper.GetRTDistance(vs1, vs2);
        return dis;
    }
    ...
    public bool ShowObbInfo(Vector3[] vs, bool isGetObbEx)
    {
        ...
        var planes = OBB.GetPlaneInfos();
        for (int i = 0; i < planes.Length; i++)
        {
            var plane = planes[i];
            if (plane.IsNaN())
            {
                Debug.LogError($"ShowObbInfo[{i}] plane.IsNaN plane:{plane}");
            }
        }
    ...
    public void TestGetObb()
    {
        ...
        OrientedBoundingBox? obb1= OrientedBoundingBox.GetObb(vs, this.name, true);
        if (obb1 == null)
        {
            return;
        }
        OBB = (OrientedBoundingBox)obb1;
    }
    ...
    public OrientedBoundingBox GetObb(Vector3[] vs,bool isGetObbEx)
    {
        ...
        if (OBB.IsInfinity())
        {
            ...
            {
                Debug.LogWarning($"GetObb Error gameObject:{this.name} vs:{vs.Length} isGetObbEx:{isGetObbEx} Extent:{OBB.Extent} ps_Last:{ps2.Last()}");
            }
            IsObbError = true;
        }
        return OBB;
    ...
    public bool GetObbEx(Vector3[] vs)
    {
        ...
        for (int i = 0; i < count && i < TestObbPointCount; i++)
        {
            ...
            if (i > 2)
            {
                ...
                if (float.IsInfinity(OBB.Extent.x))
                {
                    ...
                    sb.AppendLine($"Error[{i}/{count},{TestObbPointCount}] ps22:{ps22.Count}  ps21:{ps21.Count} Extent:{OBB.Extent} ps_Last:{ps21.Last()}");
        ...
        if (sb.Length > 0)
        {
            Debug.LogWarning($"GetObbEx go:{gameObject.name} ps:{ps22.Count}/{vs.Length}  time:{(DateTime.Now - start).TotalMilliseconds}ms OBB:{OBB} Center:{OBB.Center.Vector3ToString()} Extent:{OBB.Extent.Vector3ToString()} List:{sb.ToString()}");
        }
        else
        {
            Debug.Log($"GetObbEx go:{gameObject.name} ps:{ps22.Count}/{vs.Length}  time:{(DateTime.Now - start).TotalMilliseconds}ms OBB:{OBB} Center:{OBB.Center.Vector3ToString()} Extent:{OBB.Extent.Vector3ToString()}");
        }
        
        if (OBB.IsInfinity())
        {
            Debug.LogError($"GetObbEx Error Extent:{OBB.Extent.Vector3ToString()} ps_Last:{ps22.Last()}");
            var errorP = ps1.Last();
            CreateLocalPoint(errorP, $"ObbErrorPoint({errorP.x},{errorP.y},{errorP.z})");
        }
    ...
    public GameObject ShowOBBBox()
    {
        ...
        try
        {
            ...
            go.transform.localPosition = Vector3.zero;
            go.transform.localScale = OBB.Extent * 2f;
            go0.name += $"_{OBB.Extent.Vector3ToString3()}";
    ...
    {
        GameObject go= PointHelper.CreateLocalPoint(p, n, this.transform, lineSize);
    ...
    private GameObject CreateLocalPoint(Vector3 p, string n, Transform pT)
    {
        ...
        GameObject go = PointHelper.CreateLocalPoint(p, n, pT, lineSize);
        //go.AddComponent<DebugInfoRoot>();
        return go;
    }
    ...
    //{
    //    GameObject go = PointHelper.CreatePoint(p, n, pT, lineSize);
    //    //go.AddComponent<DebugInfoRoot>();
    //    return go;
    //}
    ...
    //        PlaneHelper.ShowPlaneInfo(this, plane, i, go, null);
    //    }
    //}

    //public void ShowPlaneInfo(PlaneInfo plane,int i,GameObject go,VerticesToPlaneInfo v2p)
    //{
    //    PlaneHelper.ShowPlaneInfo(plane,i, go, v2p, lineSize, transform);
    //}
