...
namespace MathGeoLib
{
    ...
    public struct OrientedBoundingBox
    {
        ...
        public static OrientedBoundingBox? Create(Vector3[] vs, bool isGetObbEx,string name)
        {
            ...
            if (OBB.IsInfinity())
            {
                //Debug.LogError($"GetObb Error gameObject:{name} vs:{vs.Length} isGetObbEx:{isGetObbEx} Extent:{OBB.Extent} ps_Last:{ps2.Last()}");
                var errorP = ps1.Last();
                ...
                if (isGetObbEx)
                {
                    ...
                    //    return new OrientedBoundingBox();
                    //}
                    OrientedBoundingBox? obb2 = GetObbEx(vs, name);
                    if (obb2 == null)
                    {
                        return obb2;
                    }
                    OBB = (OrientedBoundingBox)obb2; 
                }
                else
                {
                    Debug.LogError($"GetObb Error gameObject:{name} vs:{vs.Length} isGetObbEx:{isGetObbEx} Extent:{OBB.Extent} ps_Last:{ps2.Last()}");
        ...
        public static OrientedBoundingBox GetObbEx(Vector3[] vs,string name,bool isShowProgress)
        {
            DateTime start = DateTime.Now;
            List<Vector3> ps1 = new List<Vector3>();
        ...
        private static OrientedBoundingBox? GetObbEx(Vector3[] vs, object name)
        {
            ...
            if (sb.Length > 0 && isShowWarning)
            {
                Debug.LogWarning(sb.ToString());
            }

            if (ps22.Count < vs.Length / 2)
            {
                if (isShowWarning)
                {
                    Debug.LogWarning($"GetObbEx go:{name} ps:{ps22.Count}/{vs.Length}  time:{(DateTime.Now - start).TotalMilliseconds}ms OBB:{OBB} Center:{OBB.Center} Extent:{OBB.Extent}");
                }
                return null;
            }
            else
        ...
        {
            return $"[{nameof(Center)}: {Center.Vector3ToString3()}, {nameof(Extent)}: {Extent.Vector3ToString3()} Forward:{Forward}]";
        }
