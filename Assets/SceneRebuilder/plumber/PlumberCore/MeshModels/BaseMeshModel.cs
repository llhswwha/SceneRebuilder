...
{
    public virtual bool RendererModel()
    {
        return false;
    }
    ...
    public virtual void ReplaceOld()
    {
        ...
        newGo.CopyTransformMesh(model.gameObject);
    ...
    [ContextMenu("ClearGo")]
    public virtual void ClearGo()
    {
        //Debug.Log($"BaseMeshModel.ClearGo go:{this.name}");
        if (ResultGo != null)
        {
    ...
    public virtual void CheckResult()
    {
        ...
        //mcResult.MeshDistance = VertexHelper.GetVertexDistanceEx(ResultGo, this.gameObject);
        //mcResult.SizeDistance = InnerMeshRendererInfo.GetSizeDistance(ResultGo, this.gameObject);
        //mcResult.RTDistance = OBBCollider.GetObbRTDistance(ResultGo, this.gameObject);


        mcResult = CheckResult(ResultGo, this.gameObject);
        if (mcResult.SizeDistance > 0.1f)
        {
            IsGetInfoSuccess = false;
        }

        ResultGo.ClearChildren();

        Debug.Log($"CheckResult mcResult:{mcResult}");
    }

    public static PipeModelCheckResult CheckResult(GameObject newGo,GameObject oldGo)
    {
        PipeModelCheckResult mcResult = new PipeModelCheckResult();
        mcResult.ObbDistance = OBBCollider.GetObbDistance(newGo, oldGo);

        mcResult.MeshDistance = VertexHelper.GetVertexDistanceEx(newGo, oldGo);
        mcResult.SizeDistance = InnerMeshRendererInfo.GetSizeDistance(newGo, oldGo);
        mcResult.RTDistance = OBBCollider.GetObbRTDistance(newGo, oldGo);
        return mcResult;
    }
