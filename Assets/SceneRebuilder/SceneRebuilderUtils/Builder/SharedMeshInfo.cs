...
public class SharedMeshInfoList : List<SharedMeshInfo>
{
    ...
    private void InitMeshFilters(MeshFilter[] mfs)
    {
        ...
        for (int i = 0; i < mfs.Length; i++)
        {
            ...
            NoCombine b = mf.GetComponent<NoCombine>();
            if (b != null)//BoundsBox
            {
                continue;
            }
    ...
    {
        return $"v:{VertexHelper.GetVertexCountS(sharedVertexCount)} m:{this.Count}";
    }
    ...
    public SharedMeshInfo FindByMesh(Mesh mesh)
    {
        foreach(var item in this)
        {
            if (item.mesh == mesh)
...
[Serializable]
public class SharedMeshInfo:IPrefab<SharedMeshInfo>,IComparable<SharedMeshInfo>
{
    public Mesh mesh;
    ...
    public void Destroy()
    {
        ...
        {
            if (mf == null) continue;
            InnerEditorHelper.UnpackPrefab(mf.gameObject);
    ...
    internal string SaveMesh()
    {
        var mf0 = FindPrefab();
#if UNITY_EDITOR
        string assetPath = InnerEditorHelper.SaveMeshAssetResource(mf0);
    ...
    public int CompareTo(SharedMeshInfo other)
    {
