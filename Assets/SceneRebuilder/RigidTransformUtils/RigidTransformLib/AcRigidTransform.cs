...
public class AcRigidTransform : MonoBehaviour {
    ...
    public static bool RTAlign(MeshPoints mfFrom, MeshPoints mfTo, bool isShowLog = true)
    {
        DateTime start=DateTime.Now;
        ...
        VertexHelper.ClearChildren(tFrom);
        VertexHelper.ClearChildren(tTo);
        ...
        for(int l=0;l<tpsFrom.Length;l++)
        {
            ...
            for(int k=0;k<tpsTo.Length;k++)
            {
                ...
                if(dis==0)
                {
                    ...
                    //var gos= VertexHelper.ShowVertexes(vsNew,0.005f,tFrom);//0.1,0.005
                    isFound=true;
                    break;
                }
            }
